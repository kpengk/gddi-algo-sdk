...
class Raise_DustAlgo::Raise_DustAlgoPrivate {
public:
...
Raise_DustAlgo::Raise_DustAlgo(const Raise_DustAlgoConfig &config) : config_(config) {
    gddeploy::gddeploy_init("");
    private_ = std::make_unique<Raise_DustAlgoPrivate>();
...
Raise_DustAlgo::~Raise_DustAlgo() {
    for (auto &impl : private_->model_impls) { impl->WaitTaskDone(); }
}

bool Raise_DustAlgo::load_models(const std::vector<ModelConfig> &models) {
...
bool Raise_DustAlgo::sync_infer(const int64_t image_id, const cv::Mat &image, std::vector<AlgoObject> &statistic_objects) {
...
std::vector<AlgoObject> Raise_DustAlgo::parse_infer_result(const gddeploy::InferResult &infer_result,
